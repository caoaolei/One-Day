...
6. 确认后，应用结束仍在运行的任务专注计时，按照设置处理未完成任务，并保存当天复盘；自动工时不受影响，继续独立记录。
...
| 实际用时 | 由专注计时产生 |
| 完成时间 | 标记完成时记录 |
| 父任务 ID | 子任务可选关联；主任务和普通任务为空 |
| 备注 | 数据模型已预留；任务级备注编辑界面计划在 0.2 版本加入 |

支持的任务操作：

- 添加任务。
- 新建主任务时可展开子任务区域，一次添加多个子任务；单个子任务可设置预计用时并继承主任务的日期与分类。
- 今日计划中的主任务可通过操作菜单或右键菜单继续添加子任务。
- 子任务是独立任务，可单独编辑、计时、完成、恢复、移动和删除；完成后与普通任务一样进入成果档案。
- 删除主任务时保留已有子任务，并清除失效的父任务关联，避免连带丢失数据。
...
更早完成项不在看板列内折叠展开。“查看全部成果”作为看板页头的独立按钮，与“新建任务”并列，不占用任务列空间。按钮在当前 App 主内容区打开可滚动的成果档案，且包含今天完成的任务。成果档案不读取物理主屏幕尺寸，始终跟随当前窗口自适应展开，避免多显示器或窗口缩放造成偏移；日期与事项卡片采用自适应多列布局。顶部提供“按日期 / 按事项”切换，并在本次打开期间保留当前选择。
...
- 修改目标工时后，当天预计下班时间与待发送提醒立即重算；只有用户在工时页手动结束今天时才取消提醒。
- 今天页突出展示上班时间、当日工时、预计下班时间与目标进度；最新活动仅作为辅助小字显示。
- 独立“工时”页提供月度看板；每个日期展示上班时间、下班时间、当日工时与目标进度，本月顶部汇总记录天数、累计工时、日均工时和达标天数。日期卡片的绿色对勾仅表示该日工时跨度达到或超过每日目标，不表示完成复盘或手动下班。
- 月历支持切换月份和回到本月；点击有记录的日期可进入手动修正，右键任意日期可补录或修正上下班时间，页面下方保留近期明细。
- 用户可手动“结束今天”；结束后停止自动更新，并可随时选择继续记录。
- 完成今日复盘只封存任务与复盘内容，不代表下班，也不结束、锁定或禁止修正当天工时。
...
- 用户可在新建任务时展开添加多个子任务，也可从今日计划为现有主任务追加子任务。
- 子任务可独立完成和计时，并与普通任务一样出现在看板与完整成果档案中。
...
- 右键无记录日期可以补录上下班时间；再次修正同一天时更新原记录，不重复创建。
...
- 完成今日复盘后自动工时保持开启，后续活动继续更新最晚活动时间；继续记录与修正入口保持可用。
