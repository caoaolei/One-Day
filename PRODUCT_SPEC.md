...
| 看板 | 按“昨天、今天待完成、今天已完成”查看任务，支持卡片跨列拖放与历史完成列表 |
...
4. 用户填写当天复盘备注。
5. 应用按照设置处理未完成任务。
6. 保存成功后展示完成反馈弹窗，根据当天完成情况给予鼓励，并回顾完成任务数与专注时长。
...
| 分组 | 规则 |
| --- | --- |
| 昨天 | 安排在今天之前且仍未完成的任务，优先显示最近逾期项 |
| 今天待完成 | 安排到今天且未完成的任务 |
| 今天已完成 | `completedAt` 为今天的全部任务，包含今天完成的逾期任务 |

三列中的每项任务都是独立卡片，可拖到其他列。拖到“昨天”会恢复为未完成并安排到昨天；原本已经逾期的日期会保留。拖到“今天待完成”会恢复为未完成并安排到今天；拖到“今天已完成”会写入当前完成时间。同列重复拖放不会改写日期或完成时间，拖放目标在悬停时使用边框和背景高亮。

“今天已完成”顶部展示今日完成数量和实际专注时长。完成卡片使用勾选徽章、浅绿色边框、完成时间和实际用时，无计时数据时显示“手动完成”，并支持编辑、删除和恢复为未完成。任务移入该列时使用短暂淡入、缩放和勾选高光，不使用强动效。

更早完成项不在看板列内折叠展开。“查看之前完成”按钮打开独立、可滚动的历史列表，按完成日期分组并展示全部历史记录；旧数据缺少完成时间时归入“较早记录”。

看板下方另设“待安排与未来”区域，避免清除安排日期或设置未来日期后任务不可见。整个页面支持纵向滚动；未完成任务可编辑、调整日期、标记完成和删除。
...
| 勿扰提醒 | 已实现 | 不自动切换专注模式 |
| 达到预计时间提醒 | 已实现 | 系统本地通知 |
| 任务看板 | 已实现 | 昨天、今天待完成与今天已完成三列看板 |
| 看板卡片拖放 | 已实现 | 跨列同步更新安排日期和完成状态 |
| 已完成成果墙 | 已实现 | 今日摘要、恢复未完成与轻动效 |
| 历史完成列表 | 已实现 | 独立窗口按日期展示全部之前完成记录 |
| 逾期、待安排与未来任务入口 | 已实现 | 避免未完成任务从界面消失 |
| 晚间复盘 | 已实现 | 支持每日总体备注 |
...
- 点击“查看之前完成”后，能够按日期查看今天以前的全部完成记录。
