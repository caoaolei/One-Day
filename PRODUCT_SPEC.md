...
| 看板 | 按“昨天、今天、已完成”查看任务，并保留待安排和未来任务入口 |
...
| 今天 | 安排到今天且未完成的任务 |
| 已完成 | 成果墙优先展示今天完成的全部任务；更早完成默认折叠，展开后展示最近 20 条 |

成果墙顶部展示今日完成数量和实际专注时长。任务按 `completedAt` 判断完成日期，因此逾期任务只要在今天完成，也会进入今日成果；旧数据缺少完成时间时归入“更早完成”。完成卡片使用勾选徽章、浅绿色边框、完成时间和实际用时，无计时数据时显示“手动完成”，并支持编辑、删除和恢复为未完成。任务移入成果墙时使用短暂淡入、缩放和勾选高光，不使用强动效。
...
| 勿扰提醒 | 已实现 | 不自动切换专注模式 |
| 达到预计时间提醒 | 已实现 | 系统本地通知 |
| 任务看板 | 已实现 | 昨天、今天与成果墙三列看板 |
| 已完成成果墙 | 已实现 | 今日摘要、更早折叠、日期分组、恢复未完成与轻动效 |
