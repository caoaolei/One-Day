# 一日 macOS App

一个本地优先的每日任务、专注计时与复盘应用。

详细需求、产品规则和验收标准见 [PRODUCT_SPEC.md](PRODUCT_SPEC.md)，版本变化见 [CHANGELOG.md](CHANGELOG.md)。

## 已实现

- 今日任务、预计用时、完成状态与手动会议
- 新建任务可一次添加多个子任务；今日计划也可随时为现有主任务追加子任务
- 单日安排或连续 1–92 个未来工作日计划，自动跳过周末与同日同名任务
- 正向专注计时，暂停、继续和完成
- 可选的原生自动工时，默认每 1 分钟记录最早/最晚键鼠活动，午休也计入工时
- 独立工时看板，通过月历查看每天的上班、下班时间、工时与目标进度
- 自定义每日目标工时，首次活动后计算预计下班时间并在目标到达时提醒
- 工时历史、日历右键补录与修正、手动结束及登录自启动
- 看板：待安排、今天待完成与今天已完成三列，任务卡片支持跨列拖放
- 晚间复盘、备注和未完成任务处理策略
- 完成复盘前二次确认；封存后“今天 / 复盘”切换为不可编辑的日终鼓励页
- 复盘封存与自动工时相互独立，完成复盘后仍可继续记录工时并手动决定下班
- 自定义模版和日期范围批量补录
- 基于历史实际用时的自动估时
- 默认 10:30 / 20:00 的可自定义本地通知
- 首次启动设置个人称呼，之后可点击侧边栏个人区域修改
- JSON 本地持久化
- 清新晨光主题的自定义 macOS App 图标
- 逾期、未安排和未来任务统一收进看板左侧“待安排”列
- 今日成果独立展示；看板页头可直接打开完整成果档案，支持按日期或相似事项回顾
- 成果档案在当前 App 内容区内自适应展开，避免多显示器或窗口尺寸变化造成偏移
- 任务名称使用原生 macOS 输入控件，默认聚焦并正常显示空格、光标与中文输入
- 子任务是带父任务关联的独立记录，可单独估时、计时、完成并进入完整成果档案
- 成果档案使用接近全屏的大尺寸多列页面，便于集中回顾
- 成果事项可重命名、移动记录、拆分和合并，全部在本机计算并保存
- 工时月历的绿色对勾表示当日工时已达到自定义每日目标，不代表复盘完成或手动下班
- 数据损坏时先备份原文件，再创建空白数据

## 构建

需要安装包含 macOS SDK 的 Xcode 或 Command Line Tools。构建脚本会自动选择本机可用的工具链，不依赖固定 SDK 版本，也同时适配 Apple 芯片和 Intel Mac。

```bash
./scripts/build-app.sh
```

构建结果位于 `dist/一日.app`。首次保存提醒时，macOS 会请求通知权限。

构建产物适用于执行脚本的当前 Mac 架构，并使用本地临时签名，适合个人试用。

如果提示“没有找到可用的 macOS SDK”，请安装或更新 Xcode，然后执行：

```bash
sudo xcode-select --switch /Applications/Xcode.app/Contents/Developer
sudo xcodebuild -runFirstLaunch
```

任务、复盘与设置保存在 `~/Library/Application Support/YiRi/data.json`，自动工时独立保存在同目录的 `worktime.json`。

如果数据无法解析，原文件会备份为同目录下的 `data-corrupt-<时间戳>.json`，应用会展示恢复提示，不再静默覆盖。

## 测试

```bash
./scripts/run-tests.sh
```

当前 27 项回归检查覆盖计时任务切换、任务名空格、子任务关联与成果归档、看板状态迁移、未来工作日计划、成果统计与事项归组、复盘封存、复盘后继续记录工时、自动工时跨天归属、月历手动补录、最晚减最早计算、目标进度、预计下班提醒重排、结束后停止更新、最小一分钟检测间隔与工时持久化。

## 制作 DMG 安装盘

```bash
./scripts/create-dmg.sh
```

脚本会先重新构建 App，再在 `dist/` 中生成对应当前 Mac 架构的 DMG。打开安装盘后，将“一日”拖到“Applications”即可安装。

当前版本采用本地临时签名，适合内部试用。未经过 Apple Developer ID 签名和公证时，其他 Mac 第一次打开可能需要右键点击“一日”，选择“打开”。
